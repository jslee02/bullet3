...
{
	btScalar leastSquaredResidual = btSequentialImpulseConstraintSolver::solveSingleIterationNew(
		iteration,
		constraints,
		numConstraints,
		solverBodyPool,
	...
	for (int j = 0; j < multiBodyNonContactConstraints.size(); j++)
	{
	...
	for (int j0 = 0; j0 < multiBodyNormalContactConstraints.size(); j0++)
	{
...
btScalar btMultiBodyConstraintSolver::solveGroupCacheFriendlyFinishNew(
	btAlignedObjectArray<btSolverBody>* solverBodyPool,
	btConstraintArray& solverNonContactConstraintPool,
	btConstraintArray& solverContactConstraintPool,
	btConstraintArray& solverContactFrictionConstraintPool,
	...
	const btContactSolverInfo& infoGlobal)
{
	...
	int numPoolConstraints = multiBodyNormalContactConstraints.size();
	...
	//multibody joint feedback
	{
		...
		for (int i=0;i<multiBodyNonContactConstraints.size();i++)
		{
			const btMultiBodySolverConstraint& solverConstraint = multiBodyNonContactConstraints[i];
			if(solverConstraint.m_multiBodyA && solverConstraint.m_multiBodyA->isMultiDof())
			{
				applyJointFeedback(m_data,solverConstraint, solverConstraint.m_jacAindex,solverConstraint.m_multiBodyA, solverConstraint.m_appliedImpulse*btSimdScalar(1./infoGlobal.m_timeStep));
			}
	...
	numPoolConstraints = multiBodyNonContactConstraints.size();

#if 0
	//@todo: m_originalContactPoint is not initialized for btMultiBodySolverConstraint
	for (int i=0;i<numPoolConstraints;i++)
	{
		const btMultiBodySolverConstraint& c = multiBodyNonContactConstraints[i];
	...
	return btSequentialImpulseConstraintSolver::solveGroupCacheFriendlyFinishNew(
		solverBodyPool,
		solverNonContactConstraintPool,
		solverContactConstraintPool,
		solverContactFrictionConstraintPool,
